...
const supabaseUrl = 'https://sgftbfeoeaqenwdpysgq.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNnZnRiZmVvZWFxZW53ZHB5c2dxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUyMTE4NzUsImV4cCI6MjEwMDc4Nzg3NX0.dw-8d9eYUtRG8CB3VLtj2zsbp32oRa0bnfNltyo9jqk';
...
async function updateAll() {
  for (const c of data) {
    const { error } = await supabase.from('colaboradores').update({ registro: c.registro, aniversario: c.aniversario }).eq('nome', c.nome);
    if (error) console.error(`Error updating ${c.nome}:`, error);
    else console.log(`Updated ${c.nome}`);
  }
  console.log("Done");
}
